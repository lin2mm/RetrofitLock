...
出处：Session 5 — 2026-09-20（用户定义）| 状态：已被目录覆盖。现行架构是一个驱动核心、两条机械路径，不是“单一主机加配件”作为唯一结构。

### one-source-of-truth
问题：目录、图、网站三处参数打架，改一处漏两处。
做法：外壳只认 CAD 日志第一节。面盖和旋钮只认确认稿。对外句子只认已读目录。`10_product/` 只留尚未确认的商业字段，不再当唯一源。
为什么：视觉稿和数据表同时"就地编辑"必然漂移。
出处：Session 5，2026年9月20日。状态：唯一源说法已被目录和设计师定稿覆盖。
...
为什么：2026年9月20日实证。prompt 混入了当时已否的壳色、纹理和内部筋描述，旋钮也被自创形态带偏后，
