...
### two-network-paths
问题：断言外部资源不可达（说用户'别发 Drive 链接'），而用户在其他 session 一直用 Drive 链接成功
做法：任何'能不能访问 X'的判断，必须用**将来真正要用的那条执行路径**测：bash/curl 是沙盒白名单，/ 是工具侧网络，两者结论可以相反；对照实验=同一 URL 两个工具各试一次
为什么：沙盒网络与工具网络是两套出口；用沙盒 curl 的结果去回答我的工具能不能读是范畴错误，会给出反复出现的错误限制
出处：Session 5 — 2026-09-20 | 状态：有效

### drive-direct-link-intake
问题：要收外部文档，用户在用 Drive 但分享链接是预览页（只有 JS 壳，抓不到正文）
做法：要 （原文件）或 （Google Doc）；共享设为任何拥有链接的人-查看者；收到后立刻另存进  并 commit
为什么：只有导出/下载端点返回纯内容；预览页对无 JS 的抓取器等于空页；落库才持久（对话里读过不保存=又丢一次）
出处：Session 5 — 2026-09-20 | 状态：有效

### verify-artifact-not-status-label
问题：附件显示上传成功/Mark
做法：as
为什么：successful，但沙盒里找不到该文件，导致以为已收到
出处：Session 5 — 2026-09-20 | 状态：凡用户已提供的判断，用一次文件系统证据替代信任标签： 扫候选路径 + 按扩展名近时效搜索；找不到就明说没落地并立刻给替代通道
