...
## 2. 目录页面骨架
不按旧的“一款主机乘配件”重做目录。现行是一个驱动核心、两条机械路径。封面仍是 Name pending。尺寸不在本页另存。
封面      Name pending。不另选品牌。
P1 是什么  不新画产品图。若以后出图，只打开 CAD 日志第一节和旋钮日志第一节。
P2 兼容矩阵 不从已删的猜测配件行生成。门厚是询问项，不是外壳尺寸。
