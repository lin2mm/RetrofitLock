...
> Session 10：开工先看 `session10_method.md`。那里已列出可继承、待检查、可筛出。不要按本页旧「已读」标签重做或外发。
...
| `66_GTM_FrozenClosedCover_Knob_And_Envelope_Proportions_v1_2026-09-19.md` | 旋钮/包络比例记录 | — | R21：**不再作依据**（唯一正确=104+确认稿两图，`KNOB_LOG.md` §1）；原件留 Drive 只可溯源 | 已读要点 |
...
5. 沿用 R19/R20 冻结底稿与"无 logo"规则 → 你判定有问题 → 已降级 `superseded`；旋钮文字规格当时被误标作废（R18），R21 终局裁定：**不再作依据**（唯一正确=两图，`KNOB_LOG.md`）。
