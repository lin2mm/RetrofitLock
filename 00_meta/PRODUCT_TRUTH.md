...
> **2026-09-23 用户新裁决（S10-R11 明确对象）：**授权根目录中的**英文与中文两份 PDF** 是截至今天的确认目录版本，不是“一份 PDF = 一款产品”；与本页旧事实或其它历史目录冲突时，以这两个**文件实例**为准，后续仍可升级。准确文件名及仅查目录、未验 PDF 字节／产品条目的边界见 `Drive索引总结.md` §2。本页 Verified／冻结值及下面的旧铁律均不得越过此裁决；不据未读的 PDF 改写产品数字，也不从语言文件数量推导型号数量。
