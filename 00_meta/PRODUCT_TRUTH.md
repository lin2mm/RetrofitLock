# 历史项目真相档案（2026-09-20 记录 · 当前待筛选）

> **2026-09-24：**两份英中 PDF 的工具解析文本已读完。英文 2 块，中文 1 块。没有落地字节，没有 SHA-256。它们是目录语言稿，不是实验室证书。目录架构是一个驱动核心、两条机械路径：S1 保留原锁芯，S2 更换欧标锁芯。带星号的性能数字仍是工程目标。安装商页不转抄这些数字。封面占位词英中不一致，都不是已选品牌。与目录句子冲突的旧断言以目录为准，只在 `CONFLICT_LOG.md` 留一行。

## 0. 2026-09-20 历史铁律（遇今日目录冲突按上方用户新裁决）

> 用户 2026-09-20 原话：**所有以前锁定的都有问题，不能完全锁定。现在的铁律是这个最新的设计师稿件。**

**依据只有两条，分工明确：**
1. **铝合金外壳的尺寸与形状 → 只以 CAD 为准**。数字不在本页另存，只认 `CAD_LOG.md` §1。
2. **面盖（含旋钮）的外观与比例 → 只以最新设计师稿为准**（本轮附件 `image-1.png`，2026-09-20）。
   其可见特征与【问】清单见 `00_meta/CAD_LOG.md` §3c。

旧外观冻结图的文件名已从本页删除。出图只认 `CAD_LOG.md` §1 和两张确认图。
...
| 项 | 冻结值 | 状态 |
|----|--------|------|
| 品类 | 一个驱动核心、两条机械路径 | 已读目录 |
| 外壳 | CNC 铝合金，喷砂氧化。颜色只认 `CAD_LOG.md` §1 的浅银灰 | 设计师 CAD 记录 |
| 外形包络 | 不在本页另存数字。只认 `CAD_LOG.md` §1 | 设计师 CAD 记录 |
| 底部角 | 不在本页另存半径。旧的直角版和 v9 FINAL 已删。只认 `CAD_LOG.md` §1 | 设计师 CAD 记录 |
...
| 底部电池盖 | 确认稿已定：底部齐平，恰 3 道细浅横纹。旧的“待确认”和“只认三张装配截图”已删 | 设计师确认稿 |
| 品牌/UI | 确认稿有 LOGO 位。出图按 O5 的 A/B 两版。旧的“无 logo、保留与否待问”已删 | 设计师确认稿 |
