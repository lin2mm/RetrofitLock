...
**旋钮硬规则 = 现行有效**（`66_GTM_ConfirmedClosedCover_KnobSource_Crop_v1.png` 为唯一旋钮源；近直角矩形长条/≈4.30:1/接近90°端部/普通塑料/无金属套——R18 按 `102` 原文更正：此前误列 superseded；09-20 O2 终审否的只是"凸起把手"画法）→ 唯一来源 `KNOB_LOG.md` §1。
...
|------|------|------------------------|--------------|
| O2 | 顶部大圆盘 + 斜向长条 | ✅ **手动旋钮区**（功能级确认） | 旋钮形态·比例·端部·材质 = 102/70/StartHere 硬规则，唯一来源 `KNOB_LOG.md` §1；"旧规则作废"系误记已删（R18） |
...
| 旋钮 | 近直角矩形长条、≈4.30:1、接近90°端部、普通塑料、无金属套/装饰环/胶囊端 | ✅ 有效 → `KNOB_LOG.md` §1（❌superseded 判定系误记，R18 更正） |
